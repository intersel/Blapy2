...
class Blapymotion {
   ...
   */
  fadeInOut(oldContainer: HTMLElement, newContainer: HTMLElement) {
    ...
    this._fadeOut(oldContainer, fadeOutDelay, () => {
      newContainer.style.opacity = String(0)
   ...
   * @param {HTMLElement} newContainer - The container to be displayed.
   */
  rightOutIn(oldContainer: HTMLElement, newContainer: HTMLElement) {
    const fadeOutDelay = parseInt(newContainer.dataset.blapyFadeoutDelay ?? '') || 1500
    const fadeInDelay = parseInt(newContainer.dataset.blapyFadeinDelay ?? '') || 1500
   ...
   */
  _fadeOut(el: HTMLElement, duration: number, callback?: VoidFunction) {
   ...
   * @param {Function} [callback] - Optional callback after animation.
   */
  _fadeIn(el: HTMLElement, duration: number, callback?: VoidFunction) {
    el.style.transition = `opacity ${duration}ms ease`
   ...
   * @param {Function} [callback] - Optional callback after animation.
   */
  _slideOutRight(el: HTMLElement, distance: number, duration: number, callback: VoidFunction) {
   ...
   * @param {Function} [callback] - Optional callback after animation.
   */
  _slideInFromRight(el: HTMLElement, targetLeft: number, duration: number, callback: VoidFunction) {
    el.style.transition = `left ${duration}ms ease, opacity ${duration}ms ease`
