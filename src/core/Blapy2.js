...
export class Blapy {
    ...
    async initApplication() {
        ...
        try {
                ...
                PageReady: {
                    ...
                    postData: {
                        ...
                        out_function: function (p, e, data) {
                            ...
                            let urlparams = JSON.parse(JSON.stringify(data.params));
            ...
            this.myFSM.trigger = function (eventName, data) {
                /*console.log('🚨 [DEBUG]' + eventName + 'triggered from:');
                console.trace(); // ← STACK TRACE COMPLÈTE
                console.log('🚨 [DEBUG] Current FSM state:', this.currentState);
    ...
    setBlapyURL() {
        ...
        blapyLinks.forEach((bL) => {
            ...
            // Skip nested Blapy instances
            if (activeIdAttr && activeIdAttr !== this.myUIObjectID) {
                return;
            }

            let aHref;
            let tagName = bL.tagName

            // Determine the base URL depending on the element type
            switch (tagName) {
                case 'A':
                    aHref = bL.getAttribute('href');
                    break;
                case 'FORM':
                    aHref = bL.getAttribute('action');
                    break;
                default:
                    aHref = bL.getAttribute('data-blapy-href');
            }

            if (!aHref) return;

            // Append the "#blapylink" fragment if not already present
            ...
            if (tagName === 'A') {
                bL.setAttribute('href', aHref);
            } else if (tagName === "FORM") {
                bL.setAttribute("action", aHref);
            } else {
                if ((!aHref.startsWith('/') && !/^https?:\/\//i.test(aHref))) {
                    const baseEl = document.querySelector('base');
                    const aBaseHref = baseEl ? baseEl.getAttribute('href') : null;
                    if (aBaseHref) {
                ...
                bL.setAttribute('data-blapy-href', aHref);

                // Attach a click event to trigger FSM navigation
